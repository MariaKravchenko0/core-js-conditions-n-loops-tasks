...
function convertToRomanNumerals(num) {
  ...
  const arabicNumbers = [1, 4, 5, 9, 10];
  ...
  while (currentNum > 0) {
    if (currentNum >= arabicNumbers[index]) {
      result += romanNumerals[index];
      currentNum -= arabicNumbers[index];
    } else {
      index -= 1;
    }
  }
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < len / 2; i += 1) {
    for (let j = i; j < len - 1 - i; j += 1) {
      ...
      newMatrix[i][j] = newMatrix[len - 1 - j][i];
      newMatrix[len - 1 - j][i] = newMatrix[len - 1 - i][len - 1 - j];
      newMatrix[len - 1 - i][len - 1 - j] = newMatrix[j][len - 1 - i];
      newMatrix[j][len - 1 - i] = temp;
    }
  }
...
function shuffleChar(str, iterations) {
  ...
  for (let iteration = 0; iteration < iterations; iteration += 1) {
    if (cache[newStr]) {
      ...
      cache[newStr] = evenChars + oddChars;
...
function getNearestBigger(number) {
  function sortDigits(num) {
    ...
    return arrOfDigits.sort((a, b) => b - a).join('');
  }

  const maxPossibleNumber = sortDigits(number);

  for (let i = number + 1; i <= maxPossibleNumber; i += 1) {
    if (sortDigits(i) === maxPossibleNumber) {
      return i;
    }
  }
